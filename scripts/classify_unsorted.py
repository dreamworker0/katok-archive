...
# LLM 호출은 scripts/llm.py 로 옮겼다. 이 파일 안에 있는 동안 일곱 모듈이 여기서
# import 했고(태그 하나 나누는 스크립트가 분류기 1,204줄을 끌고 들어왔다), 분류기를
# 고칠 때마다 관계없는 모듈이 함께 흔들렸다. `jsonio.py` 때와 같은 상황이라 같은
# 방식으로 정리했다 — 옮기고, 예전 이름은 여기 남긴다. 부르는 곳도 검사도 이 이름을
# 쓰고 있어서다.
...
- 내용이 이어지면 한 스레드로 묶고, 무관하면 나눕니다. 한 스레드가 될 수도 있고
  여러 개가 될 수도 있습니다.
- **바로 앞 메시지에 붙이기 전에 내용 단서를 먼저 보세요.** 카톡의 답장 기능으로
  몇 시간 전 메시지에 답하는 일이 흔한데, 내보내기에는 '누구에게 답장'이 남지
  않습니다. 시간이 가깝다는 것만으로 묶지 말고, 말 속 단서(번역·논문·영상 같은
  대상, 언어, 호칭)가 어느 글을 가리키는지로 판단하세요.
  · 실측 2026-08-06: 번역해서 읽어보겠다는 답장이 직전의 한국어 유튜브 영상에
    붙었다 — 실제로는 세 시간 전 영어 논문 공유에 단 답장이었다.
...
def merge_graph(knowledge: dict, graph: dict,
    ...
    보던 때는 뜻이 안 되는 엣지가 원장에 남았다 — 실측 2026-08-14 에
