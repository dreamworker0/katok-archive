...
**2026-09-22 이후로는 이것이 첫 수단이 아니다.** scripts/kakao_replies.ps1 이
방 창 화면에서 '○○에게 답장' 머리글과 인용문을 직접 건져 오므로, 스크롤백이 닿는
구간은 추론이 아니라 **사실**로 확정된다(output/replies.jsonl → apply_replies.py).
여기는 그 구간 **밖** — 스크롤백이 사라진 옛 날짜 — 을 위한 보조로 남는다.
확정된 답장이 있는 구간에 이것을 돌려 의심 목록을 만들면, 사실 위에 추측을 덧씌우게
된다. 돌리기 전에 output/replies.jsonl 이 그 날짜를 덮는지 먼저 볼 것.
...
출력: output/audit-thread-fit.json — **통째로 덮어쓴다.** --days 로 하루만 돌리면
그 전 결과가 전부 날아간다(실측: 2026-08-06 에 쌓은 73건). 이어 쓰려면 먼저 복사해 둘 것.
