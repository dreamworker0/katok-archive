...
뜻이 안 되는 엣지가 원장에 남았다 — 실측 2026-08-14 에 `person -belongs-> topic`
...
# 12분면에서 표본이 너무 얇아, 실측 2026-08-14 에 32명 중 5명이 관심 분야가 0개였다.
#
...
#
# 왜 이 둘인가
#     큰 분류만 나눈다. 한 카드에 백 개를 늘어놓으면 목록이 아니라 벽이고, 요지
#     산문도 한 문단이 그것을 다 덮는다.
#
#     projects 100개 → 갈래 여덟(실측 2026-09-04: 33 · 13 · 13 · 13 · 11 · 8 · 5 · 3).
#     ai-tools 54개 → 갈래 여섯. 이쪽 축은 사물이 아니라 **무엇을 하려 했나**다
#     (에이전트 구축 · 지침과 스킬 · 토큰 살림 · 연결과 배포 · 도구 고르기 ·
#     새 도구 소식). 도구 이름으로 가르면 안 되는 이유가 실측에 있다 —
#     '안티그래비티' 하나가 41편, '클로드 코드' 가 13편에 걸쳐 있고 한 편에 도구가
#     셋씩 나온다. 그것으로 나누면 묶음이 아니라 또 하나의 목록이 된다.
#
#     그래서 도구 이름은 갈래 밑에 넣지 않았다(`config/tag_broader.json`). 넣으면
#     그 도구가 나온 대화 전부가 한 갈래로 끌려온다 — 2026-08-21 `adopt_orphans`
#     사고와 같은 꼴이다.
#
#     남은 열 분류에서 `DIGEST_SECTION_FROM`(20) 을 넘는 것은 events 41 ·
#     ai-models 33 · welfare-practice 32 · community 21 · governance 21 이다.
#     그 다섯은 갈래 없이 시기·화두로 절을 나눈다(`topic_reports.DIGEST_RULES`) —
#     무엇으로 가를지 사람이 표에 적기 전에는 코드가 할 일이 없다.
CATEGORY_FACETS = {"projects": "앱 제작", "ai-tools": "AI 코딩 도구"}
...
#
# 실측 2026-08-14: 이 걸림이 없을 때 발행본에서 세 사람이 '일상·잡담' 을 관심 분야로
...
def load_node_tags(path: Path | None = None) -> dict[str, list[str]]:
    ...
    '다룬 주제' 가 0개가 된다 — 실측 2026-08-14 에 앱 78개 중 38개.
