...
#
# `(?<!\d\.)` — **소수점 뒤는 주민번호가 아니다.** 발행본을 통째로 json 으로
# 늘어놓고 검사하므로 숫자 리터럴도 검사 대상에 들어온다. 실측 2026-09-09:
# 관계망 노드의 크기값 `22.3707055437449` 에서 소수부 열세 자리가 그대로
# `370705-5437449` 로 읽혀 밤 갱신이 '테스트' 단계에서 멈췄다. 소수부 안쪽에서
# 시작하는 자리는 앞이 숫자라 `(?<!\d)` 가 이미 막으므로, 소수점 바로 뒤만
# 더 막으면 소수부 전체가 빠진다.
_P_RRN = re.compile(
    r"(?<!\d)(?<!\d\.)(\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01]))" + _SEP +
    r"?([1-8]\d{6})(?!\d)")
