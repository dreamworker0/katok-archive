...
- 상태: **끝. 2026-09-04 에 0~6단계를 실행하고 적재·배포까지 마쳤다.** 다시 돌리지 말 것 —
  1·2단계는 원장을 바꾸는 일이고 3단계는 12편에 $12.7 이 든다. 아래 4장의 프롬프트는
  같은 일을 또 하라는 지시가 아니라 **무엇을 왜 그렇게 했나의 기록**으로 읽는다.
...
| 항목 | 계획 시작 | 끝 |
|---|---|---|
| 요지 12편 정리 시점 | 2026-07-28 · `as_of` 없음 | 12편 모두 2026-09-04 · `as_of` 주제 수 일치 |
| 절이 있는 편 | 0 | 9 (`DIGEST_SECTION_FROM` 미만인 셋은 규칙대로 없음) |
| 갈래 없는 projects 주제 | 43 | 1 |
| projects 갈래 | 7개 | 8개 — '개인·가족 앱' 신설 |
| news-articles / chat | 27 / 15 | 19 / 12 |
| projects | 106 | 100 |
| 요지 태그 중 이어지지 않아 뺀 것 | 1 | 0 |
| 검사 | 858 | 933 |

- 새로 생긴 것: `scripts/digest_prose.py`(밤 갱신 5d) · `scripts/audit_form_categories.py` ·
  `split_tag --fill-category` · `topic_reports.DIGEST_*` · `ontology.CATEGORY_FACETS` ·
  `build_site.facet_groups` · 요지 화면의 묶음 nav·절·정리 시점
- 커밋: `33dfe06` `449fbb8` `80db310` `3ef5899`. 데이터 변화는 커밋이 아니라 적재로 나갔다.
- 계획과 달라진 것 하나: 2단계 목표가 '갈래 없는 projects 5 이하' 였는데 첫 채우기로는 15가
  남았다. 원인은 갈래 표에 자리가 없던 것이어서 갈래를 하나 세우고(개인·가족 앱) 표의
  빈칸을 메운 뒤 1이 되었다. 남은 1편(t-415)은 아래 '남는 것' 참고.
- 5단계(태그 잔여)는 계획대로 **제안서만** 냈다: `output/adopt-proposal-20260904.json`.
...
- ~~`news-articles` 가 작아졌으면 분류 id 를 거둘지 결정~~ → **2026-09-04 결정: 거두지 않는다.**
  1단계로 27 → 19 가 되었지만 셋을 실측하고 그렇게 정했다.
  1. **19편은 작지 않다.** hwp(12)·chat(12)보다 크다. 그리고 그 19편은 1단계가 27편을
     전부 읽고 **남는 것이 맞다고 판정한 것**이다 — 자료를 여러 건 늘어놓은 것, 설명
     없는 링크, 묶는 주제가 없는 것. 억지로 옮기면 받는 분류의 요지가 틀려진다.
     이 계획이 고치려던 병을 반대 방향으로 다시 앓는 셈이다.
  2. **보조 분류로 걸린 16편의 곁길을 함께 잃는다.** '뉴스로 공유된 논문'(t-200)처럼
     주 분류를 옮기고 보조로 남겨 둔 길이고, 그 길을 만든 것이 1단계다.
  3. **들어올 것도 있다.** t-415 는 정의상 `news-articles` 인데 방벽에 막혀 못 갔다.
     들어올 것이 있는 분류를 거두는 것은 이르다.

  즉 **27 → 19 는 병이 줄어든 것이고, 분류가 쓸모없어진 것이 아니다.** 다시 물을 때는
  이 셋이 달라졌는지부터 본다.
- ~~ai-tools 50개의 묶음~~ → **2026-09-04 끝.** 축은 '무엇을 하려 했나' 여섯 갈래로
  적었다(에이전트 구축 13 · 도구 고르기 8 · 새 도구 소식 8 · 토큰 살림 7 · 지침과 스킬 6 ·
  연결과 배포 6 · 그 밖 6). `ontology.CATEGORY_FACETS` 에 `ai-tools` 를 더했고 커밋은
  `be0933c` 다. 표를 적을 때 지킨 것 둘이 다음 분류에도 그대로 적용된다 — **도구·결과물
  이름은 갈래 밑에 넣지 않는다**(그 이름이 나온 대화 전부가 끌려온다), **이미 부모가 있는
  태그는 옮기지 않는다**(태그는 이름 붙은 부모를 하나만 가진다).
- 관계망 근거는 별도 계획서로 옮겼다 → [PLAN-graph-evidence-2026-09.md](PLAN-graph-evidence-2026-09.md)

### 이번에 처리하지 않고 남긴 두 편

- **t-415**(웹접근성 평가 도구·정부 UIUX 가이드라인) — 남이 만든 도구·문서 소개라 정의상
  `news-articles` 인데, 1단계가 형식 분류를 **목적지로 쓰지 못하게** 막아 두었다
  (`audit_form_categories.FORM_CATEGORIES`). 그 방벽은 모델이 형식 분류끼리 주제를 돌리는
  것을 막으려고 세운 것이고, 사람이 판단한 한 건을 막는 것은 뜻하지 않은 결과다.
  지금은 projects 의 '그 밖' 한 편으로 남아 있고 보조 분류에 `news-articles` 가 있어
  자료 쪽에서도 찾힌다. 옮기려면 그 방벽을 사람이 푸는 판단이 필요하다.
- **t-048**(부산 사회복지사협회 구글 워크스페이스 도입) — welfare-practice 로 옮겼으나 보조
  분류가 이미 `governance`·`infra` 로 꽉 차(`assign_secondary.MAX_PER_THREAD`=2) 옛 분류
  `projects` 를 못 붙였다. 그 주제에는 결과물이 없어 projects 로 찾을 이유도 적으므로
  그대로 두는 것이 맞다고 보았다.
